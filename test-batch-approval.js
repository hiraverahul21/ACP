...
async function testBatchApprovalLogic() {
  try {
    console.log('🧪 Testing batch approval logic...');
    ...
    // 3. Find an item with available batch
    ...
    // 5. Create a test material issue
    const testQty = Math.min(5, batch.current_qty);
    ...
    const issue = await prisma.materialIssue.create({
      data: {
        ...
        purpose: 'Testing batch approval logic',
    ...
    console.log(`   - Quantity: ${testQty}`);
    console.log(`   - Expected behavior: ${existingDestBatch ? 'Add to existing batch' : 'Create new batch entry'}`);
    
    console.log('\n🎯 Now test the approval in the UI to verify batch handling!');
